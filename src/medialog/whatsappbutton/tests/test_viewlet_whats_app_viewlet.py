# -*- coding: utf-8 -*-
from medialog.whatsappbutton.interfaces import IMedialogWhatsappbuttonLayer
from medialog.whatsappbutton.testing import MEDIALOG_WHATSAPPBUTTON_FUNCTIONAL_TESTING
from medialog.whatsappbutton.testing import MEDIALOG_WHATSAPPBUTTON_INTEGRATION_TESTING
...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_whats_app_viewlet_is_registered(self):
        view = BrowserView(self.portal['other-document'], self.request)
        manager_name = 'plone.abovecontenttitle'
        alsoProvides(self.request, IMedialogWhatsappbuttonLayer)
        ...
        manager.update()
        my_viewlet = [v for v in manager.viewlets if v.__name__ == 'whats-app-viewlet']  # NOQA: E501
        self.assertEqual(len(my_viewlet), 1)

    # XXX would be nice to have this test working:
    # def test_whats_app_viewlet_is_not_available_on_newsitem(self):
...
class ViewletFunctionalTest(unittest.TestCase):
    ...
    layer = MEDIALOG_WHATSAPPBUTTON_FUNCTIONAL_TESTING
